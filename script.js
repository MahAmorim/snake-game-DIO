window.addEventListener("DOMContentLoaded", function (event) {
  ...
  window.addEventListener("keydown", function (event) {
    ...
    if (event.key == "H" || event.key == "h") {
      ...
      noteElement.innerHTML = `Modo hard. Aperte espaço para começar!`;
    ...
    if (event.key == "E" || event.key == "e") {
      ...
      noteElement.innerHTML = `Modo simples. Aperte espaço para começar!`;
      noteElement.style.opacity = 1;
      resetGame();
      return;
    }
  ...
  // The main game loop - this function gets invoked approximately 60 times per second to render the game
  ...
  function main(timestamp) {
    try {
      ...
      const pressSpaceToStart = "Aperte espaço para começar!";
      const changeMode = hardMode
        ? "Voltar para o modo simples? Aperte Shift + E"
        : "Pronto para mais? Aperte Shift + H";
      const followMe =
        'Siga me <a href="https://github.com/MahAmorim" , target="_blank">@MahAmorim</a>';
  ...
  function stepAndTransition(percentageOfStep) {
    ...
    // Start with tail instead of head cause the head might step into the previous position of the tail
    ...
    if (newHeadPosition != applePosition) {
      ...
      // Set up and start transition for new tail - make sure it heads to the right direction and set initial size
  ...
  function getNextPosition() {
    ...
    switch (snakeDirection) {
      ...
      case "up": {
        const nextPosition = headPosition - width;
        if (nextPosition < 0) throw Error("A cobrinha bateu na parede");
        // Ignore the last snake part, it'll move out as the head moves in
        if (snakePositions.slice(1).includes(nextPosition))
          throw Error("A cobrinha se mordeu");
        return nextPosition;
